...
- Backup Script: [`scripts/backup-to-gdrive.sh`](file:///c:/Users/mdikr/Documents/CODE/Monsur Ali Travels_Fullstack/scripts/backup-to-gdrive.sh)
- Output Folder on VPS: `/opt/dev/backups`
- Log File on VPS: `/var/log/monsuralitravels-backup.log`
- Target Container: `monsuralitravels-mongodb-dev`
...
1. Automatically targets container `monsuralitravels-mongodb-dev` (or reads `.env`).
2. Creates compressed database dump in `/opt/dev/backups/monsuralitravels_db_perfume-store_<timestamp>.gz`.
3. Uploads the compressed file to Google Drive under folder `Monsur Ali Travels_DB_Backups`.
...
```cron
0 3 * * * /opt/dev/scripts/backup-to-gdrive.sh >> /var/log/monsuralitravels-backup.log 2>&1
```
...
```bash
tail -f /var/log/monsuralitravels-backup.log
```

To list backups stored on Google Drive:
```bash
rclone ls gdrive:Monsur Ali Travels_DB_Backups
```
