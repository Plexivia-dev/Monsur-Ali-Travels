...
This document describes how the white-label dashboard manages client-specific configuration settings (themes, brand logos, menu permissions, and route access guards) for Monsur Ali Travels, Engulfic, and Toyland.
...
└── clientConfig/
    ├── 01monsuralitravels/
    │   └── config.json       # Monsur Ali Travels configuration
    ├── 02engulfic/
...
3. **Default:** Defaults to `'monsuralitravels'` (Client ID: `01`).

The resolver is exported from [`src/clientConfig/index.js`](file:///c:/Users/mdikr/Documents/CODE/Monsur Ali Travels_Fullstack/dashboard/src/clientConfig/index.js):
```javascript
...
The [`clientThemeProvider.jsx`](file:///c:/Users/mdikr/Documents/CODE/Monsur Ali Travels_Fullstack/dashboard/src/components/core/clientThemeProvider.jsx) reads the configuration `theme` key and dynamically injects them into a `<style>` block in the document header at runtime:
...
The [`appSidebar.jsx`](file:///c:/Users/mdikr/Documents/CODE/Monsur Ali Travels_Fullstack/dashboard/src/components/core/dashboard/appSidebar.jsx) uses a `hasAccess` helper function to filter rendered menu items:
```javascript
...
To prevent users from typing off-limit paths directly in the browser address bar (e.g., `/dashboard/billing`), we wrap layout outlets in the [`clientRouteGuard.jsx`](file:///c:/Users/mdikr/Documents/CODE/Monsur Ali Travels_Fullstack/dashboard/src/components/core/clientRouteGuard.jsx) component. It checks the active path, extracts the required permission, and redirects users to `/dashboard` if access is denied.
...
```bash
# Run Monsur Ali Travels Dev environment
npm run dev:monsuralitravels
