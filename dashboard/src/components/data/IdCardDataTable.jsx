...
import { Search, RefreshCw, UserCheck, Trash2 } from 'lucide-react';
...
export function IdCardDataTable() {
  ...
  const fetchData = async (page = 1, limit = pagination.limit, searchQuery = search, statusFilter = status) => {
    try {
      ...
      const res = await apiClient.get('/api/v1/id-cards', { params });
      ...
      toast.error('আইডি কার্ড তালিকা লোড করতে সমস্যা হয়েছে।');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleDelete = async (id, name) => {
    if (!window.confirm(`আপনি কি আইডি কার্ড "${name || id}" মুছে ফেলতে চান?`)) return;
    try {
      await apiClient.delete(`/api/v1/id-cards/${id}`);
      toast.success('আইডি কার্ড মুছে ফেলা হয়েছে।');
      fetchData(pagination.page, pagination.limit, search, status);
    } catch (err) {
      console.error('Failed to delete ID card:', err);
      toast.error('আইডি কার্ড মুছতে সমস্যা হয়েছে।');
    }
  };
      ...
      <div className="bg-card border border-border p-5 rounded-xl shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-lg font-bold text-foreground tracking-tight flex items-center gap-2">
            ...
            Employee ID Cards (কর্মচারী আইডি কার্ড তালিকা)
          </h2>
          <p className="text-xs text-muted-foreground mt-0.5">
            প্রতিষ্ঠানের সকল কর্মকর্তা ও কর্মচারীদের ডিজিটাল আইডি কার্ড ডাটাবেজ রেকর্ড।
          </p>
        </div>

        <button
          type="button"
          onClick={() => switchPortal('docs', 'idcard')}
        ...
        >
          <span>+ নতুন আইডি কার্ড তৈরি</span>
        </button>
      </div>
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={9} className="text-center py-10 text-muted-foreground font-medium">
                    কোনো আইডি কার্ড রেকর্ড পাওয়া যায়নি।
                  </td>
                </tr>
              ) : (
                data.map((item, idx) => (
                  <tr key={item._id || idx} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="p-3 font-mono text-muted-foreground">
                      {formatToDdMmYyyy(item.joiningDate) || '—'}
                    </td>
