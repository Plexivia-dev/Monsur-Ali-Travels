...
import { SAMPLE_INVOICE } from './sampleData';
import { Download, RefreshCw, Eye, Edit3, Columns } from 'lucide-react';

export function InvoiceBuilder() {
  const [data, setData] = useState(SAMPLE_INVOICE);
  const [viewMode, setViewMode] = useState('split');
  const [isExportOpen, setIsExportOpen] = useState(false);

  const handleResetSample = () => {
    setData(SAMPLE_INVOICE);
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 bg-card border border-border p-3 rounded-xl shadow-xs">
        ...
        <div className="flex items-center space-x-2">
          <button
            ...
            title="Reset Sample Invoice Data"
          >
      ...
      {viewMode === 'preview' && (
        <div>
          <InvoicePreview data={data} onPrint={() => setIsExportOpen(true)} />
        </div>
      )}
