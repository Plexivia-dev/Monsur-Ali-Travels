  ...
  return (
    <div className="w-full flex flex-col items-center">
      ...
      <PrintablePaper id="printable-invoice-canvas">
        <div className="flex-1 flex flex-col justify-between text-slate-900 min-h-[960px] print:min-h-0 print:h-full print:justify-between">
          
          <div className="space-y-5 print:space-y-3.5 flex-1">
            {/* Header Biller Info & Document Title */}
            <div className="flex flex-col sm:flex-row justify-between items-start border-b-2 border-slate-900 pb-4 print:pb-3 gap-4">
              ...
              <div className="text-right space-y-1.5 print:space-y-1">
                <div className="inline-block bg-slate-900 text-white px-4 py-1 print:px-3 print:py-0.5 rounded-[4px] text-lg print:text-sm font-black uppercase tracking-wider">
                  INVOICE
                </div>
                
                <div className="text-xs print:text-[11px] font-mono text-slate-800 space-y-0.5">
                ...
                <div className="pt-0.5">
                  {paymentStatus === 'Paid' && (
                    <span className="inline-flex items-center gap-1 bg-emerald-100 text-emerald-900 border border-emerald-400 text-xs print:text-[10px] font-black px-2.5 py-0.5 rounded-[4px]">
                      <CheckCircle className="w-3.5 h-3.5 text-emerald-600" /> PAID
                    </span>
                  )}
                  {paymentStatus === 'Pending' && (
                    <span className="inline-flex items-center gap-1 bg-amber-100 text-amber-900 border border-amber-400 text-xs print:text-[10px] font-black px-2.5 py-0.5 rounded-[4px]">
                      <Clock className="w-3.5 h-3.5 text-amber-600" /> PENDING
                    </span>
                  )}
                  {paymentStatus === 'Overdue' && (
                    <span className="inline-flex items-center gap-1 bg-rose-100 text-rose-900 border border-rose-400 text-xs print:text-[10px] font-black px-2.5 py-0.5 rounded-[4px]">
            ...
            <div className="border border-slate-900 rounded-[4px] overflow-hidden text-sm print:text-xs">
              <table className="w-full text-left border-collapse border border-slate-900">
                <thead>
                  <tr className="bg-slate-900 text-white uppercase text-xs print:text-[10px] font-bold">
                    <th className="p-2.5 print:p-1.5 w-10 text-center border border-slate-900">#</th>
                    <th className="p-2.5 print:p-1.5 w-48 border border-slate-900">Item Title</th>
                    <th className="p-2.5 print:p-1.5 border border-slate-900">Description</th>
                    <th className="p-2.5 print:p-1.5 text-center w-20 border border-slate-900">Qty</th>
                    <th className="p-2.5 print:p-1.5 text-right w-28 border border-slate-900">Rate ({currency})</th>
                    <th className="p-2.5 print:p-1.5 text-right w-32 border border-slate-900">Total ({currency})</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-300">
                  {displayItems.map((item, idx) => (
                    <tr key={item.id || idx} className="hover:bg-slate-50/80 font-medium h-9 print:h-7">
                      <td className="p-2.5 print:py-1 print:px-1.5 text-center text-slate-500 font-mono border border-slate-300">{idx + 1}</td>
                      <td className="p-2.5 print:py-1 print:px-1.5 text-slate-900 font-bold border border-slate-300">{item.isEmpty ? '—' : (item.title || 'Service Item')}</td>
                      <td className="p-2.5 print:py-1 print:px-1.5 text-slate-800 border border-slate-300">{item.isEmpty ? '—' : (item.description || '—')}</td>
                      <td className="p-2.5 print:py-1 print:px-1.5 text-center font-mono font-bold text-slate-900 border border-slate-300">{item.qtyDisplay}</td>
                      <td className="p-2.5 print:py-1 print:px-1.5 text-right font-mono border border-slate-300">{item.isEmpty ? '-' : (parseFloat(item.unitPrice) || 0).toLocaleString('en-IN')}</td>
                      <td className="p-2.5 print:py-1 print:px-1.5 text-right font-mono font-bold text-slate-900 border border-slate-300">{item.isEmpty ? '-' : item.lineTotal.toLocaleString('en-IN')}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start gap-4 print:gap-3 pt-1">
              <div className="max-w-md text-xs print:text-[11px] space-y-1">
                <span className="font-bold text-slate-700 uppercase tracking-wider text-xs print:text-[10px]">Payment Terms & Notes:</span>
                <p className="bg-slate-50 p-2.5 print:p-2 rounded-[4px] border border-slate-300 text-slate-700 leading-relaxed text-xs print:text-[10px]">
                  {paymentTerms || 'Payment due within 15 days of invoice date.'}
                </p>
              </div>

              <div className="w-full sm:w-72 space-y-1 text-sm print:text-xs font-mono border-t border-slate-400 pt-1.5">
                ...
                <div className="flex justify-between text-base print:text-sm font-bold text-slate-900 border-t-2 border-slate-900 pt-1.5 bg-slate-100 p-2 rounded-[4px]">
