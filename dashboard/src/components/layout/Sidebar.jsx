    ...
    <SidebarPrimitive collapsible="icon" className="border-r border-border bg-sidebar transition-all duration-300">
      ...
      <SidebarContent className="p-2 space-y-4">
        {navGroups.map((group, groupIdx) => {
          const isCurrentPortal = activePortal === group.portal;
                          ...
                          <SidebarMenuItem>
                            <CollapsibleTrigger asChild>
                              <SidebarMenuButton
                                tooltip={item.label}
                                className={cn(
                                  'w-full justify-between cursor-pointer font-medium text-xs',
                                ...
                                {item.childItems.map((subItem, subIdx) => {
                                  ...
                                  return (
                                    <SidebarMenuSubItem key={subIdx}>
                                      <SidebarMenuSubButton
                                        isActive={isActive}
                                        onClick={() => switchPortal(subItem.portal, subItem.submodule)}
                                        className={cn(
                                          'cursor-pointer text-xs rounded-md py-1.5 transition-colors',
                                          isActive
                    ...
                    return (
                      <SidebarMenuItem key={itemIdx}>
                        <SidebarMenuButton
                          isActive={isActive}
                          tooltip={item.label}
                          onClick={() => switchPortal(item.portal, item.submodule)}
                          className={cn(
                            'cursor-pointer text-xs font-medium rounded-lg transition-colors',
                            isActive
