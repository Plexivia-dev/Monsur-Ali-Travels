    ...
    <div className="space-y-6">
      ...
      {viewMode === 'preview' && (
        <div className="space-y-6">
          ...
          <div className="no-print bg-card border border-border p-4 sm:p-5 rounded-xl shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h2 className="text-base font-bold text-foreground tracking-tight flex items-center gap-2">
                ...
                Passport Intake Document Ready (Tracking ID:{' '}
                <span className="font-mono text-emerald-600 font-bold">{data.trackingNo}</span>)
              </h2>
              <p className="text-xs text-muted-foreground mt-0.5">
                The passport file is logged and verified. You can print, download PDF, share on WhatsApp, or edit details.
              </p>
            </div>
          ...
          {/* Printable A4 Canvas */}
          <PassportSubmissionPreview data={data} onPrint={handlePrint} />
        </div>
      )}
    </div>
  );
