...
export function DashboardLayout({
  ...
  return (
    <SidebarProvider defaultOpen={true}>
      <div className={cn('flex h-screen w-full overflow-hidden bg-background text-foreground transition-colors', className)}>
        {/* Navigation Sidebar */}
        {sidebar}
        ...
        <SidebarInset className="flex flex-1 flex-col min-w-0 h-screen overflow-hidden bg-background">
          ...
          {/* Dynamic Content View Container (Height strictly 100vh - 56px, internally scrollable) */}
          <main
            className={cn(
              'flex-1 overflow-y-auto overflow-x-hidden h-[calc(100vh-3.5rem)] max-h-[calc(100vh-3.5rem)] p-4 sm:p-6 lg:p-8 w-full max-w-[1440px] mx-auto space-y-6',
              mainClassName
            )}
          >
            {children}
