...
export function ClientCaseFiles() {
  const { user } = useAuth();
  ...
  const fetchClients = async () => {
    ...
    try {
      ...
      const rawClients = candRes.status === 'fulfilled' ? (candRes.value.data?.data || candRes.value.data || []) : [];
      ...
      const normalizedClients = (Array.isArray(rawClients) ? rawClients : []).filter(c => !existingDids.has(c._id || c.did)).map(c => ({
        _id: c._id || c.did,
        did: c.did,
      ...
      setCases([...normalized, ...normalizedClients]);
    } catch (err) {
      console.error('Failed to load client case files:', err);
  ...
  const handleCreateCase = async (e) => {
    e.preventDefault();
    if (!newForm.clientName || !newForm.passportNumber) {
      toast.error('Client Name and Passport Number are required.');
      return;
    }

    setSubmitting(true);
    try {
      const response = await apiClient.post('/api/v1/client/clients', newForm);
      ...
      toast.error(err.response?.data?.message || 'Failed to create client case file.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      {/* Top Banner Header */}
      <PageTitle
        title="Client Case Files & Agent Pipeline"
        description="Track 5-stage deployment pipelines for overseas manpower clients, manage destination agency allocations, local sub-agencies, and live MongoDB document verification vaults."
        icon={Globe2}
        actions={
          <>
            <button
      ...
      <div className="bg-card border border-border rounded-xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="relative w-full sm:w-80">
          ...
          <input
            type="text"
            placeholder="Search client name, passport, trade skill..."
            value={searchQuery}
      ...
      {loading ? (
        <div className="py-20 flex flex-col items-center justify-center space-y-3">
          ...
          <p className="text-xs text-muted-foreground font-medium">Fetching Client Case Files from Database...</p>
        </div>
      ) : filteredCases.length === 0 ? (
        <div className="bg-card border border-border rounded-xl p-12 text-center space-y-3">
          <FolderOpen className="w-12 h-12 text-muted-foreground/40 mx-auto" />
          <h3 className="text-sm font-bold text-foreground">No Client Case Files Found</h3>
          <p className="text-xs text-muted-foreground max-w-sm mx-auto">
            No clients matched your search criteria or filter. Create a new case file to get started.
          </p>
        </div>
      ) : (
...
export default ClientCaseFiles;
